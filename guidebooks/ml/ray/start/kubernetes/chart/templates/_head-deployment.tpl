    ...
    spec:
      ...
      containers:
        - name: ray-head
          ...
          args:
          {{ if .Values.workdir }}
            - {{ print "set -e; set -o pipefail; " (include "ray.head.args" .) "; mkdir /tmp/workdir; echo $RAY_WORKDIR_ENCODED | base64 -d | tar -C /tmp/workdir -jxf -; if [ ! -f /tmp/workdir/runtime-env.yaml ]; then touch /tmp/workdir/runtime-env.yaml; if [ -f /tmp/workdir/requirements.txt ]; then echo 'Splicing in requirements.txt'; awk 'BEGIN { print \"pip:\" } NF > 0 { print \"  - \" $0 }' /tmp/workdir/requirements.txt >> /tmp/workdir/runtime-env.yaml; fi; fi ; ls -l /tmp/workdir ; ray job submit --job-id " (.Values.jobId) " --address http://localhost:8265 --working-dir=/tmp/workdir --runtime-env=/tmp/workdir/runtime-env.yaml -- " (.Values.commandLinePrefix | default "python3 main.py") " $(echo " (.Values.dashdash | default "") " | base64 -d); echo 'Job complete, shutting down ray...'; ray stop" }}
          {{ else }}
