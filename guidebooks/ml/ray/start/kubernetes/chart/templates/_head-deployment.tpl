    ...
    spec:
      ...
      containers:
        - name: ray-head
          ...
          command: [ "/bin/bash", "-c", "--" ]
          args:
            - {{ print "if ! $(which ray); then pip install ray[default]==" .Values.failsafes.ray.version "; fi; ray start --head --port=6379 --redis-shard-ports=6380,6381 --num-cpus=" .Values.podTypes.rayHeadType.CPUInteger " --num-gpus=" .Values.podTypes.rayHeadType.GPU " --object-manager-port=22345 --node-manager-port=22346 --dashboard-host=0.0.0.0 --block" }}
          ports:
