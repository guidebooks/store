...
if [ "$KUBE_POD_MANAGER" = mcad ] || [ "$KUBE_POD_MANAGER" = kubernetes ]; then
    ...
    BRANCH=${RAY_CHART_BRANCH-1.11.0} # <-- will get updated by @release-it/bumper; see top-level package.json
...
NUM_CPUS_INTEGER=$(echo ${NUM_CPUS-250m} | awk 'function ceil(x, y){y=int(x); return(x>y?y+1:y)} /[^m]$/ { print $1 } /m$/ { sub("m",""); print ceil($1.11.0) }')
