...
cd "$CUSTOM_WORKING_DIR" && \
    torchx run --workspace="" \
        ...
        | sed -E 's#(python -m torch.distributed.run|torchrun)#export TERM=xterm-256color; if [ -f /tmp/configmap/workdir/workdir.tar.gz ]; then echo "Unpacking workspace with PYTHONPATH=$PYTHONPATH"; mkdir /tmp/workdir; tar -C /tmp/workdir -zxvf /tmp/configmap/workdir/workdir.tar.gz; fi; cd /tmp/workdir; trap "code=$?; if [[ $code = 0 ]]; then status=Succeeded; msg=\\"Job completed successfully\\"; else status=Failed; msg=\\"Job failed\\"; fi; echo \\"$(tput bold)$(tput dim)$(tput setaf 2)[Job $(tput sgr0)$(tput setaf 2)${status}$(tput bold)$(tput dim)$(tput setaf 2)] $(tput sgr0)$(tput dim)$(tput setaf 2)pod/$(hostname) ${msg} $(tput sgr0)$(tput setaf 2)$(date -u +\\"%Y-%m-%dT%T.%3NZ\\")$(tput sgr0)\\" | tee -a /tmp/status.txt; exit $code" EXIT; (sleep 4 \&\& echo "$(tput bold)$(tput dim)$(tput setaf 2)[Job $(tput sgr0)$(tput setaf 2)Running$(tput bold)$(tput dim)$(tput setaf 2)] $(tput sgr0)$(tput dim)$(tput setaf 2)pod/$(hostname) Job is active $(tput sgr0)$(tput setaf 2)$(date -u +"%Y-%m-%dT%T.%3NZ")$(tput sgr0)" | tee -a /tmp/status.txt ) \& \1#' \
