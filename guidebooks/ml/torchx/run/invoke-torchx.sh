...
cd "$CUSTOM_WORKING_DIR" && \
    torchx run --workspace="" \
        ...
        | sed -E 's#(python -m torch.distributed.run|torchrun)#export TERM=xterm-256color; if [ -f /tmp/configmap/workdir/workdir.tar.gz ]; then echo "Unpacking workspace"; mkdir /tmp/workdir; tar -C /tmp/workdir -zxvf /tmp/configmap/workdir/workdir.tar.gz; fi; cd /tmp/workdir; function log() { status="$1"; msg="$2"; echo -e "\\x1b[2;1;32m[Job \\x1b[0;32m${status}\\x1b[1;2;32m] \\x1b[0;2;32mpod/$(hostname) ${msg} \\x1b[0;32m$(date -u +%Y-%m-%dT%T.%3NZ)\\x1b[0m" | tee -a /tmp/status.txt ; } ; function catch() { code=$?; log Failed "Job failed"; exit $code; } ; trap catch ERR; function bye() { if [[ $? = 0 ]]; then log Succeeded "Job completed successfully"; fi; } ; trap bye EXIT; (sleep 4 \&\& log Running "Job is active" ) \& \1#' \
